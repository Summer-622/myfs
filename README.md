# 基于 FUSE 的 EXT2 文件系统 (MyFS) - C++ 重构版
...
系统支持标准的文件操作，包括创建文件/目录、读写数据、硬链接、重命名、截断文件以及权限管理等，并通过了所有的功能测试点 (34/34)。
...
* **解决**: 利用 `std::getline(ss, token, '/')` 实现流式分割，代码量减少 60%。

---

## 📊 实验结果

系统成功通过了脚本中的所有测试用例，涵盖挂载、目录创建、文件读写、权限检查、重命名及卸载等场景。

![测试结果截图](./assets/result_screenshot.png)
